...
The subject took `r format(sum(daily.ttl$steps), big.mark = ",")` steps over the course of `r num.days` days. This is an average of `r format(round(daily.avg, 0), big.mark = ",")` steps per day. The median number of steps per day was `r format(daily.med, big.mark = ",")`. The difference between the average and median number of steps per day indicate the data is skewed.

Exploring the data further, the histogram below shows `r under1000` days, or `r paste0(round(percent.under, 1), "%")` percent of days, had 0 to 999 total steps recorded. This indicated a potential problem with the data which is explored and addressed in the "Imputing missing values" section below.
...
The 5 minute interval `r max.interval$start.time` to `r max.interval$end.time` was, on average, the period with the most steps, as illustrated in the timeseries plot above.
...
Missing data was recorded as _NA_. There were `r format(ttl.missing, big.mark = ",")` intervals with missing step data, including `r days.all.missing` days where all interverals were missing.
...
During the week, the 5-minute interval `r mod.max.interval$start.time[1]` to `r mod.max.interval$end.time[1]` was, on average, the period with the most steps, with an average of `r mod.max.interval$avg.steps[1]` steps. By contrast, on weekends, the 5 minute interval `r mod.max.interval$start.time[2]` to `r mod.max.interval$end.time[2]` was, on average, the period with the most steps, with an average of `r mod.max.interval$avg.steps[2]` steps.
